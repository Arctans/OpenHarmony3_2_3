...
/*CONFIG_USING_KERNEL_DTB表示是否使用kernel设备树初始化uboot启动阶段，
add by chenhongsheng 2023.11.09*/
/* #define CONFIG_USING_KERNEL_DTB */
#ifdef CONFIG_USING_KERNEL_DTB
extern int dm_uninit(void);
extern int dm_init_and_scan(bool pre_reloc_only);
#include <boot_imximg.h>
#endif
...
#define ENET_PAD_CTRL (PAD_CTL_SPEED_HIGH |			\
	PAD_CTL_DSE_40ohm | PAD_CTL_HYS)

#define ENET_TX_PAD_CTRL  (PAD_CTL_DSE_40ohm | PAD_CTL_HYS)

#define ENET_RX_PAD_CTRL  (PAD_CTL_PUS_100K_UP | PAD_CTL_DSE_40ohm | PAD_CTL_HYS)

#define ENET_REF_CLK_PAD_CTRL  (PAD_CTL_SPEED_HIGH |		\
...
#define I2C_PMIC	0	/* modify by yeyicheng pmic use spi1 2018-10-10 */
...
//ut059e
#define DISP0_PWR_EN		IMX_GPIO_NR(1,21)
#define DISP1_LCDC_BL		IMX_GPIO_NR(2,16)
#define DISP1_LCDC_CTL		IMX_GPIO_NR(4,11)
/* 2020-6-5 add by zhouhuadong hardware_watchdog */
#ifdef CONFIG_HW_WATCHDOG

//初始化位0 或者初始化 会出现卡死现象
static int hw_watchdog_init_done = -1;
static int hw_watchdog_trigger_level = -1;

//lwl add @20211220
#define WDT_GPIO_PORT   5
#define WDT_GPIO_NUM    9
#define WDT_TIME_OUT  50  

#define WDT_GPIO5_DR		0x20AC000 
#define WDT_GPIO5_GDIR		0x20AC004
#define WDT_GPIO5_PSR		0x20AC008
#define WDT_GPIO5_ICR1      0x20AC00C
#define WDT_GPIO5_ICR2		0x20AC010 
#define WDT_GPIO5_IMR		0x20AC014
#define WDT_GPIO5_ISR		0x20AC018
#define WDT_GPIO5_EDGE_SE	0x20AC01C



static iomux_v3_cfg_t const wdg_pads[] = {
	//IOMUX_PADS(PAD_DISP0_DAT0__GPIO4_IO21 | MUX_PAD_CTRL(UART_PAD_CTRL)),
	IOMUX_PADS(PAD_DISP0_DAT15__GPIO5_IO09 | MUX_PAD_CTRL(NO_PAD_CTRL)),
};

static void setup_iomux_wdg(void)
{
	SETUP_IOMUX_PADS(wdg_pads);
};

void hw_watchdog_reset(void)
{

	if(hw_watchdog_init_done < 0)
		return;
...
//		value = readl(WDT_GPIO5_DR);
//		value |= hw_watchdog_trigger_level<<9;
//	
//		value = (value & (~(1<<9)) )| (hw_watchdog_trigger_level<<9);
//	
//		writel(value,WDT_GPIO5_DR);
//	}
	

	//gpio_set_value(IMX_GPIO_NR(4, 21), hw_watchdog_trigger_level);
	gpio_set_value(IMX_GPIO_NR(WDT_GPIO_PORT, WDT_GPIO_NUM), hw_watchdog_trigger_level);
}
#if 1
...
void hw_watchdog_init(void)
{
	...
	puts("    hw_watchdog_init!!!\n");	
	setup_iomux_wdg();
	gpio_request(IMX_GPIO_NR(WDT_GPIO_PORT, WDT_GPIO_NUM), "gpio_watchdog");
	gpio_direction_output(IMX_GPIO_NR(WDT_GPIO_PORT, WDT_GPIO_NUM), 0);
	hw_watchdog_reset();
	hw_watchdog_init_done = 1;
	//gpio_direction_output(IMX_GPIO_NR(WDT_GPIO_PORT, WDT_GPIO_NUM), 1);
...
static void enable_backlight(void)
{
	...
	gpio_request(DISP1_LCDC_BL, "Display LCDC_BL");
	gpio_request(DISP1_LCDC_CTL, "Display LCDC_CTL");

	gpio_direction_output(DISP1_LCDC_CTL, 0);	
	udelay(1);
	//gpio_direction_output(DISP1_LCDC_CTL, 1);	

	gpio_direction_output(DISP1_LCDC_BL, 0);	
	gpio_direction_output(DISP0_PWR_EN, 0);
	//udelay(1000);
	//gpio_direction_output(DISP1_LCDC_BL, 1);
...
#if 1	/*modify by yeyicheng 2018-11-14*/
#if 1	/*lvds模式*/
...
static void setup_display(void)
{
	...
	reg = IOMUXC_GPR2_BGREF_RRMODE_EXTERNAL_RES
	     ...
	     | IOMUXC_GPR2_LVDS_CH1_MODE_DISABLED            //lwl change lvds1 to lvds0
...
static void setup_fec(void)
{
	...
	/* 	/1* select ENET MAC0 TX clock from PLL *1/ */
...
/*使用kernel设备树文件初始化uboot，add by chenhongsheng 2023.11.09*/
#ifdef CONFIG_USING_KERNEL_DTB 
    ret = init_kernel_dtb();
	if(ret == 0 )
	{
...
int power_init_board(void)
{
	...
	if (!is_mx6dqp()) {
		...
		reg &= ~LDO_VOL_MASK;
		//reg |= LDOB_3_00V;
		reg |= LDOB_2_50V;  //lwl 修改因为 phy 是2.5V的电压
