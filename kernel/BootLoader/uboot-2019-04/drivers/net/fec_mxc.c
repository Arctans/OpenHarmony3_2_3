...
创建日期：  2023-06-15
函数说明：  yt8521 初始化函数.
参数说明：  udevice 对应的net设备
返回值  ：  无.
修改记录：  .
*/
static int ytphy_init(struct udevice *dev)
{
	int ret;
	struct device_node *np = &(dev->node.np);
	struct fec_priv *fec = dev_get_priv(dev);
	static int init_once = 0;
	char *data;
	int len,i;
	struct ytphy_ext_reg{
		u16 reg;	
		u16 val;
	}*yt_reg;
	...
	/* lwl add 20230403*/ 
	/* enable tx_delay clock when no wire plug */
	if(fec->phydev->phy_id == PHY_ID_YT8521)
	{

		data = fdt_getprop(gd->fdt_blob, dev_of_offset(dev), "ytphy-init-regs",
			       &len);
		/* data = (char*)of_get_property(np, "ytphy-init-regs",&len); */
		if(!data)
		{
			return -EINVAL;
		}
		yt_reg = (struct ytphy_ext_reg*)malloc(len);
		if(!yt_reg)
		{
			return -ENOMEM;
		}
		for(i = 0;i<len/4;i++)
		{
			...
			yt_reg[i].val  =  *(data++)<<8;
			yt_reg[i].val |=  *data++;
			/* printk("the reg is 0x%0x val 0x%0x\n",yt_reg[i].reg,yt_reg[i].val); */
			ret = ytphy_write_ext(fec->phydev, yt_reg[i].reg, yt_reg[i].val);
		}
